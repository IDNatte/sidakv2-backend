from app.model import User, DynamicData, Organization, SectoralGroup, OrgDetail, OrgDetailBanner
from flask import Blueprint, jsonify, request, g, abort, current_app, send_from_directory
from app.helper.auth import authentication, allowed_file, allowed_file_img
...
def add_header(response):
    response.headers['Access-Control-Allow-Methods'] = common_config.HTTP_HEADER_CONFIG.get(
        'METHODS')
    response.headers['Access-Control-Allow-Headers'] = common_config.HTTP_HEADER_CONFIG.get(
        'HEADERS')
    response.headers['Access-Control-Allow-Origin'] = common_config.HTTP_HEADER_CONFIG.get(
        'ORIGIN')
    response.headers['Content-Security-Policy'] = common_config.HTTP_HEADER_CONFIG.get(
        'CSP')
    response.headers['X-Content-Type-Options'] = common_config.HTTP_HEADER_CONFIG.get(
        'XCTPO')
    response.headers['X-Frame-Options'] = common_config.HTTP_HEADER_CONFIG.get(
        'XFO')
    response.headers['X-XSS-Protection'] = common_config.HTTP_HEADER_CONFIG.get(
        'XSSP')
